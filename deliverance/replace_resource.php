...
//		or die(mysql_error());
	...
	$resource = mysql_fetch_array($result);
		...
		$_SESSION['site'] = $resource['site'];
			...
			// assign site for history tracking
			$site = $resource['site'];
	...
	include("settings.php");
		...
		// first, remove everything but the filename itself from the path stored in the db
		// currently, the first 31 characters need to go away -> /uploads/deliverance_resources/
		$fileNameOnly = substr($resource['filePath'], 31);

		// then assign full delete path to session
		$_SESSION['fileToDelete'] = $_SESSION['deletePath'] . $fileNameOnly;
...
<?php
	...
	if (!mysql_query($query,$DBlink)) {
		...
		echo '<br /><p>There was a problem saving the changes to the database. Please email <a href="mailto:kmbeyer@email.arizona.edu">kmbeyer@email.arizona.edu</a> for assistance. The resource was replaced, but the history was not saved.</p>';
			
		## EMAIL
		// BEGIN: prepare to send the error notification email
		$to = 'kmbeyer@email.arizona.edu, samarketingcritical@gmail.com';
		$subject = 'ERROR: DELIVERANCE PROCESSOR SCRIPT: SAVE HISTORY FAILED';
		$body = "There was a problem saving the Deliverance history to the database on " . date('m/d/Y H:i:s') . ". The resource was replaced, but the history was not saved. This script was executed on the " . $server . "."; 
		// END: prepare to send the error notification email
