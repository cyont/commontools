...
// if coming from the full resource listing/search page, put the GET var into the POST var
if ($_GET['resourceID']) {
	$_POST['resource'] = $_GET['resourceID'];
}

// if "X" has been clicked, delete the resource's entry in the sequentialfeed table
if ($_GET['deleteResource']) {
	
	// execute the delete
	$result = mysql_query("DELETE FROM sequentialfeed WHERE id = " . $_GET['deleteResource'] . "");
...
if ($_POST['assignResource']) {
	...
	if ($_POST['makeDefault']) {
		...
		if ($default) {
			...
			if (!mysql_query($query,$DBlink)) {
				...
				$subject = 'ERROR: SA MARKETING CMS ADD DEFAULT RESOURCE TO SEQUENTIAL DISPLAY BLOCK';
		...
		// there's no default
			...
			if (!mysql_query($query,$DBlink)) {
				...
				// BEGIN: prepare to send the error notification email
				$to = 'kmbeyer@email.arizona.edu';
				$subject = 'ERROR: SA MARKETING CMS SEQUENTIAL DISPLAY DEFAULT RESOURCE ADDITION';
	...
	// this is a new data entry, not a default
		...
		if (!empty($startDate) && !empty($endDate) && ($startDate <= $endDate) && ($startDate >= $today)) {
			...
			$result = mysql_query("SELECT * FROM sequentialfeed WHERE displayBlockID = " . $_GET['displayBlockID'] . " AND  endDate >= '" . $today . "' ORDER BY endDate ASC");
		...
		if ($rangeOK && !$dateError) {
			...
			$query = "INSERT INTO sequentialfeed
			...
			if (!mysql_query($query,$DBlink)) {
				...
				// BEGIN: prepare to send the error notification email
				$to = 'kmbeyer@email.arizona.edu';
				$subject = 'ERROR: SA MARKETING CMS ADD RESOURCE TO SEQUENTIAL DISPLAY BLOCK';
				$body = "There was a problem saving the resource information to the database on " . date('m/d/Y H:i:s') . "."; 
				// END: prepare to send the error notification email
...
if ($_SESSION['allowEdits']) {
	...
	// QUERY: get sequential feed info
	$resultSequential = mysql_query("SELECT * FROM sequentialfeed WHERE displayBlockID = " . $_GET['displayBlockID'] . " AND (startDate = '" . $today . "' OR endDate >= '" . $today . "') ORDER BY startDate "); // changed from endDate
...
<title>Add Resource to Sequential Display Block ~ Deliverance ~ Student Affairs ~ The University of Arizona</title>
...
<!--		<a href="#" title="Add a resource to this display block"><img src="/deliverance/images/plus_sign.gif" width="13" height="13" alt="Add a resource to this display block" id="plusSign" /></a>-->
    ...
    <h2>Deliverance</h2>
	<h3>Add Resource to Sequential Display Block <span style="color:#F00">#<? echo $rowDisplay['id']; ?></span></h3>
	...
	<br class="clear" />
		...
		// look through all the results for sequential resource queues
		while ($rowSequentialResources = mysql_fetch_array($resultSequential)) {
			
			// if this queue hasn't started yet, set a flag
			if ($rowSequentialResources['startDate'] > $today) {
				$future = true;
			}
			...
			$result = mysql_query("SELECT * FROM resource WHERE " . $rowSequentialResources['resourceID'] . " = id");
		...
		<!-- BEGIN group -->
			...
			<div class="resourceImgInfo">
			<p><strong<? if ($future) { echo ' style="color:#70adcd"'; } ?>>Queue</strong> [ <a href="add_resource_sequential_display_block.php?deleteResource=<? echo $rowSequentialResources['id'] . '&displayBlockID=' . $_GET['displayBlockID']; ?>" title="Remove resource from this display block during all dates associated with this queue.">X</a> ]<br />
			
			<span class="resourceDates">
			<?
				// assign the start and end dates to vars
				$startDate = $rowSequentialResources['startDate'];
				$endDate = $rowSequentialResources['endDate'];
				
				// reformat start dates for smaller footprint in queue column
				$startDateShort = strtotime($rowSequentialResources['startDate']);
				$startDateShort = date('m/d', $startDateShort);
				
				// reformat end dates for smaller footprint in queue column
				$endDateShort = strtotime($rowSequentialResources['endDate']);
		...
		<?
		} // END WHILE ($rowSequentialResources = mysql_fetch_array($resultSequential))
		?>
	...
	?>
		...
		<div id="resRun">
		<p style="margin-bottom:0;"><strong>Resources &amp; Run Dates</strong>&nbsp;&nbsp;<a href="add_resource_sequential_display_block_search.php?displayBlockID=<? echo $_GET['displayBlockID'] ?>" title="If you are not seeing the resource you want in the pulldown, use this link to access the full and searchable list of available resources. The pulldown shows only the most recently uploaded resources."><img src="images/search.png" alt="If you are not seeing the resource you want in the pulldown, use this link to access the full and searchable list of available resources. The pulldown shows only the most recently uploaded resources." title="If you are not seeing the resource you want in the pulldown, use this link to access the full and searchable list of available resources. The pulldown shows only the most recently uploaded resources." border="0" align="absmiddle" /></a></p>

		<?php
	
	// QUERY: get all appropriate resources (2/3/2011 added range limit)
	$resultResMatch = mysql_query("SELECT * FROM resource WHERE dimensionsID = " . $rowDisplay['dimensionsID'] . " AND site = '" . $rowDisplay['site'] . "' AND uploadDate >= '" . $resourceRangeSequential . "' ORDER BY resourceName");
	...
	<a href="upload_resource.php?dimensions=<? echo $rowDisplay['dimensionsID'] ?>&site=<? echo $rowDisplay['site'] ?>">Upload a new resource</a>
	...
	?>
		...
		<div id="calBlockInner">
			...
			?>
				...
				<ul>
					<?php
					include("inc_sequential_cal.php");
					?>
				</ul>
				</div><!-- /dayBlock -->
			</div><!-- /day -->
		...
		<div style="text-align:center"><a href="add_resource_sequential_display_block.php?subtractWeek=true&displayBlockID=<? echo $_GET['displayBlockID']; ?>">Previous</a> | <a href="add_resource_sequential_display_block.php?displayBlockID=<? echo $_GET['displayBlockID']; ?>">Current</a> | <a href="add_resource_sequential_display_block.php?addWeek=true&displayBlockID=<? echo $_GET['displayBlockID']; ?>">Next</a></div>
