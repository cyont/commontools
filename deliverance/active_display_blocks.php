  ...
  <div id="mainContent">
	...
	if ($campusRec) {
		...
		if ($_POST['sort']) {
			if ($_POST['sort'] == 'displayBlockID') {
				$query = mysql_query("SELECT * FROM pages WHERE domain LIKE '%campusrec%' AND path NOT LIKE '%get_deliv%' ORDER BY displayBlockID " . $_POST['order'] . "");
			} else if ($_POST['sort'] == 'location') {
				$query = mysql_query("SELECT * FROM pages WHERE domain LIKE '%campusrec%' AND path NOT LIKE '%get_deliv%' ORDER BY domain, path " . $_POST['order'] . "");
			} else if ($_POST['sort'] == 'type') {
				$query = mysql_query("SELECT * FROM pages WHERE domain LIKE '%campusrec%' AND path NOT LIKE '%get_deliv%' ORDER BY type " . $_POST['order'] . "");
			} else if ($_POST['sort'] == 'date') {
				$query = mysql_query("SELECT * FROM pages WHERE domain LIKE '%campusrec%' AND path NOT LIKE '%get_deliv%' ORDER BY date " . $_POST['order'] . "");
			}
		} // END IF ($_POST['sort'])
	...
	if (!$_POST || $_POST['sort'] == '') {
		...
		if ($campusRec) {
			...
			// DEFAULT QUERY
			$query = mysql_query("SELECT * FROM pages WHERE domain LIKE '%campusrec%' AND path NOT LIKE '%get_deliv%' ORDER BY domain ASC, path ASC");
