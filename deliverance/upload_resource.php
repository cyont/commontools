...
// define page so that correct values are received from settings.php
$page = 'upload';

## BEGIN HISTORY TRACKING
// define vars for saving history
$action = 'upload';
$time = time();
$user = strtoupper($_SESSION['webauth']['netID']);

// check domain for test server (deliverance.test) and adjust server and site vars accordingly
...
*/
		...
		if ($_POST['site']) {
			...
			if (in_array($_FILES['resourceFile']['type'], $allowed)) {
				...
				$_SESSION['fileName'] = cleanFileName($_FILES['resourceFile']['name']);
				...
				if (move_uploaded_file ($_FILES['resourceFile']['tmp_name'], $_SESSION['uploadPath'] . $filePrefix . cleanFileName($_FILES['resourceFile']['name']))) {
				} // End of move... IF.
		...
		if ($_FILES['resourceFile']['error'] > 0) {
			...
			switch ($_FILES['resourceFile']['error']) {
				case 1:
					...
					break;
				case 2:
					$error_msg['resourceFile'] = $errorPrefix . 'it exceeds ' . $uploadMaxText . '.'; // this is the max size defined in the form
					break;
				case 3:
...
## Pre-populate the form with what we know about the resource
// if we're showing the editable form, check the GET vars to see if we know what site and what dimension to pre-populate
// if we're NOT, then clear out the SESSION vars so we don't get default values in a blank upload form
if ($_GET['dimensions']) {
	$_SESSION['dimensions'] = $_GET['dimensions'];
} else if (!$_GET['dimensions'] && !$_POST) {
	$_SESSION['dimensions'] = '';
}

if ($_GET['site']) {
	$_SESSION['site'] = $_GET['site'];
} else if (!$_GET['site'] && !$_POST) {
	$_SESSION['site'] = '';
}
...
?>
	...
	<!-- RESOURCE NAME -->
	<span<?php echo $error_class['resourceName']; ?>><strong>Resource Name:</strong></span><span style="color:#F00">*</span>
	...
	<!-- DIMENSIONS -->	
	<span<?php echo $error_class['dimensions']; ?>><strong>Dimensions (W x H in px):</strong></span><span style="color:#F00">*</span>
	...
	<!-- SITE -->
	<span<?php echo $error_class['site']; ?>><strong>Website:</strong></span><span style="color:#F00">*</span>
	...
	<!-- ALT TEXT -->
	<span<?php echo $error_class['altTxt']; ?>><strong>Alt text:</strong></span><span style="color:#F00">*</span>
	...
	<!-- FILE TO UPLOAD -->
	<span<?php echo $error_class['resourceFile']; ?>><strong>File to Upload</strong><span style="color:#F00">*</span> <span style="font-size:9px;">(jpg, gif, png / &lt; <? echo $uploadMaxText; ?>):</span></span>
	...
	<!-- <? echo $uploadMaxText; ?> upload size limit/server limit is 2 MB --><input name="MAX_FILE_SIZE" value="<? echo $uploadMaxFile; ?>" type="hidden" /><input name="resourceFile" type="file" class="upload" /><br />&nbsp;<br />
<?php 
		}
?>
	<!-- LINK -->
...
<?php
	...
	(filePath, fileSize, dimensionsID, type, resourceName, resourceLink, headline, subtext, site, altTxt, description, uploadDate)
	VALUES ('$filePath', $fileSize, $dimensionsID, '$type', '$resourceName', '$resourceLink', '$headline', '$subtext', '$site', '$altTxt', '$description', '$uploadDate')";
	...
	(netID, action, server, resourceName, filePath, site, timestamp)
	VALUES ('$user', '$action', '$server', '$resourceName', '$filePath', '$site', $time)";

	// check for errors saving to the db
	if (!mysql_query($query,$DBlink)) {

		echo '<br /><p>There was a problem saving the changes to the database. Please email <a href="mailto:kmbeyer@email.arizona.edu">kmbeyer@email.arizona.edu</a> for assistance. The resource was uploaded, but the history was not saved.</p>';
			
		## EMAIL
		// BEGIN: prepare to send the error notification email
		$to = 'kmbeyer@email.arizona.edu, samarketingcritical@gmail.com';
		$subject = 'ERROR: DELIVERANCE PROCESSOR SCRIPT: SAVE HISTORY FAILED';
		$body = "There was a problem saving the Deliverance history to the database on " . date('m/d/Y H:i:s') . ". The resource was uploaded, but the history was not saved. This script was executed on the " . $server . "."; 
		// END: prepare to send the error notification email
