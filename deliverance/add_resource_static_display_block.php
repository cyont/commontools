...
<!--		<div style="position:absolute; bottom:2px; right:2px;"><a href="edit_display_block_type.php?displayBlockID=<?// echo $rowDisplay['id']; ?>" title="WARNING: This will erase all resources in the display block queue!">Edit display type</a></div>-->
	...
	$resultResMatch = mysql_query("SELECT * FROM resource WHERE dimensionsID = " . $rowDisplay['dimensionsID'] . " AND site = '" . $rowDisplay['site'] . "' AND uploadDate >= '" . $resourceRangeStatic . "' ORDER BY resourceName");
	
	?>

	<div style="float:left;">
	<form enctype="multipart/form-data" action="<?= $_SERVER['PHP_SELF'] . '?displayBlockID=' . $_GET['displayBlockID']; ?>" method="POST">
		<select name="resource" onChange="this.form.submit();" style="width:140px;">
		...
		?>
		</select>&nbsp;&nbsp;<a href="add_resource_static_display_block_search.php?displayBlockID=<? echo $_GET['displayBlockID'] ?>" title="If you are not seeing the resource you want in the pulldown, use this link to access the full and searchable list of available resources. The pulldown shows only the most recently uploaded resources."><img src="images/search.png" alt="If you are not seeing the resource you want in the pulldown, use this link to access the full and searchable list of available resources. The pulldown shows only the most recently uploaded resources." title="If you are not seeing the resource you want in the pulldown, use this link to access the full and searchable list of available resources. The pulldown shows only the most recently uploaded resources." border="0" align="absmiddle" /></a><br />&nbsp;<br />
	</form>
