...
if (($_COOKIE['Deliverance'] == 'allowEdits' || $_COOKIE['Deliverance_test'] == 'allowEdits') && stristr($_SERVER['SERVER_NAME'], 'satest') === false && stristr($_SERVER['SERVER_NAME'], 'sutest') === false && stristr($_SERVER['SERVER_NAME'], 'rctest') === false) {
	...
	echo '<div style="z-index:100; top:20px; right:20px; position:absolute; font-family:Arial; font-size:11px;"><a href="' . $_SERVER['PHP_SELF'] . '?edit=on" style="cursor:pointer;">edit</a> | <a href="' . $_SERVER['PHP_SELF'] . '?edit=off" style="cursor:pointer;">browse</a></div>';
...
global $dbdeliv;
		...
		$result = $dbdeliv->query("SELECT resourceID FROM current WHERE displayBlockID = " . $displayBlockID . "");
		$row = mysqli_fetch_array($result);
		
		// if NULL returned, get the default value for this display block
		if (!$row) {
			$result = $dbdeliv->query("SELECT resourceID FROM defaults WHERE displayBlockID = " . $displayBlockID . "");
			$row = mysqli_fetch_array($result);
		} // END IF for default check
		
		// QUERY: get the resource parameters based on its ID
		$result = $dbdeliv->query("SELECT * FROM resource WHERE id = " . $row['resourceID'] . "");
		$row = mysqli_fetch_array($result);
		...
		if ($_GET['edit'] != 'on') {
			...
			if (!$row['resourceLink']) {
				...
				echo '<img src="' . $row['filePath'] . '" title="' . $row['altTxt'] . '" alt="' . $row['altTxt'] . '" border="0" />';
			} else {
				// display the linked image
				echo '<a href="' . $row['resourceLink'] . '"><img src="' . $row['filePath'] . '" title="' . $row['altTxt'] . '" alt="' . $row['altTxt'] . '" border="0" /></a>';
			} // END IF for link check
		
		// if edit mode is on, then link the display block to the display block editor
		} else {
			// global $editURL;
			echo '<a href="' . $editURL . '/deliverance/add_resource_static_display_block.php?displayBlockID=' . $displayBlockID . '" style="cursor:pointer;"><img src="' . $row['filePath'] . '" title="EDIT: STATIC" alt="EDIT: STATIC" border="0" style="cursor:pointer; opacity:0.35; filter:alpha(opacity=35)" onmouseover="this.style.opacity=.65; this.filters.alpha.opacity=65" onmouseout="this.style.opacity=0.35; this.filters.alpha.opacity=35" /></a>';
...
global $dbdeliv;
		...
		$result = $dbdeliv->query("SELECT resourceID FROM current WHERE displayBlockID = " . $displayBlockID . " ORDER BY RAND() LIMIT 1");
		$row = mysqli_fetch_array($result);
		...
		// if edit mode is on, then link the display block to the display block editor
			...
			// global $editURL;
			echo '<a href="' . $editURL . '/deliverance/add_resource_random_display_block.php?displayBlockID=' . $displayBlockID . '" style="cursor:pointer;"><img src="' . $row['filePath'] . '" title="EDIT: RANDOM" alt="EDIT: RANDOM" border="0" style="cursor:pointer; opacity:0.35; filter:alpha(opacity=35)" onmouseover="this.style.opacity=.65; this.filters.alpha.opacity=65" onmouseout="this.style.opacity=0.35; this.filters.alpha.opacity=35" /></a>';
		} // end IF check for edit mode on
...
global $dbdeliv;
		...
		// if edit mode is on, then link the display block to the display block editor
			...
			// global $editURL;
			echo '<a href="' . $editURL . '/deliverance/add_resource_sequential_display_block.php?displayBlockID=' . $displayBlockID . '" style="cursor:pointer;"><img src="' . $row['filePath'] . '" title="EDIT: SEQUENTIAL" alt="EDIT: SEQUENTIAL" border="0" style="cursor:pointer; opacity:0.35; filter:alpha(opacity=35)" onmouseover="this.style.opacity=.65; this.filters.alpha.opacity=65" onmouseout="this.style.opacity=0.35; this.filters.alpha.opacity=35" /></a>';
		} // end IF check for edit mode on
